...
## Result
The synthesis results show that both the case-based and if–elseif 32-bit ALUs are functionally identical and synthesizable. The case design uses 1293 cells while the if–elseif ALU uses 1242 cells, but their total areas are nearly the same (10254.481 µm² vs 9913.876 µm²). Power values are also very close, with logic power around 7.6×10⁻⁴ W for the case model and 7.54×10⁻⁴ W for the if–elseif model. Internal power remains almost equal (5.047×10⁻⁴ W vs 5.022×10⁻⁴ W) and switching power differs only slightly (2.183×10⁻⁴ W vs 2.164×10⁻⁴ W). Overall, both architectures show very similar area and power performance with only minor variations.
